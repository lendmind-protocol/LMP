...
| IPFS distribution adapter | `crates/lmp-sync/`, `orchestrator/registry_release_gate.py` | Productized locally / mixed external state | One current package CID is gateway-verified; three recorded pins were detected as stale and downgraded to pending | Republish the three current packages and rerun the manifest/CID gate |
