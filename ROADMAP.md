...
| OCI registry transport | `crates/lmp-sync/`, `packages/registry/` | Productized locally / external pending | Real OCI manifest/blob/signature paths are tested against a mock Distribution endpoint; public registry publication is not claimed | Complete a credentialed public registry qualification before release publication |
| IPFS distribution adapter | `crates/lmp-sync/`, `orchestrator/registry_release_gate.py` | Productized locally / external pending | Local Kubo retrieval and CID verification are recorded; public pinning is intentionally not claimed without an external pin | Run a credentialed public pin and gateway retrieval qualification before release publication |
| Rust language parser | `crates/lmp-core/src/ast.rs`, `crates/lmp-core/src/evaluator.rs` | Productized locally / bounded | `syn` parser and AST checks are tested; unsupported languages are reported fail-closed | Add parser-backed adapters before claiming coverage for additional languages |
...
| Docker sandbox | `orchestrator/sandbox.py`, `orchestrator/qualification_suite.py` | Productized locally / optional | Network, filesystem, capability, memory, PID, timeout, and redaction controls are covered by qualification tests | Keep opt-in and retain independent host/image qualification as a release gate |
...
| Release workflow | `.github/workflows/release.yml` | Productized locally / external pending | Cross-platform builds, checksums, archive safety, and publication gates are implemented | Complete public immutable distribution and independent review gates before publishing |
| Profile definitions | `registry/minds/`, `profiles/` | Productized locally / curation-bound | 11 checked-in Mind packages validate with signed manifests, provenance metadata, rule contracts, fixtures, and limitations | Add independently sourced profiles through the proposal/review path; do not inflate the catalog with generated placeholders |
