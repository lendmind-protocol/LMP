...
| 13 — Optional sandbox and remote distribution | Productized locally, hosted qualification verified | Docker, local OCI/IPFS paths, GitHub Pages deployment, and all four hosted release targets plus aggregate archive verification pass; three current public IPFS packages are gateway-verified, while one package remains pending republish. |
...
| OCI registry transport | `crates/lmp-sync/`, `packages/registry/` | Productized locally / external pending | Real OCI manifest/blob/signature paths are tested against a mock Distribution endpoint; no public OCI artifact is recorded | Complete a credentialed public registry qualification before release publication |
| IPFS distribution adapter | `crates/lmp-sync/`, `orchestrator/registry_release_gate.py` | Productized locally / mixed external state | Three current package CIDs are gateway-verified; one package remains pending | Republish the remaining package and rerun the manifest/CID gate |
