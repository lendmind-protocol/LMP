...
| 13 — Optional sandbox and remote distribution | Productized locally, hosted qualification verified | Docker, local OCI/IPFS paths, GitHub Pages deployment, and all four hosted release targets plus aggregate archive verification pass; all four current public IPFS packages are gateway-verified. OCI publication and release evidence remain separate gates. |
...
| IPFS distribution adapter | `crates/lmp-sync/`, `orchestrator/registry_release_gate.py` | Productized locally / externally gateway-verified | All four current package CIDs are gateway-verified | Preserve the recorded CIDs and rerun the gate whenever a package revision changes |
