...
`syn-2` for Rust 2021 syntax. The TypeScript evaluator uses
`ts-morph/typescript-compiler-api` for TypeScript and JavaScript AST traversal.
...
MiB memory ceiling. If Docker or the required image is unavailable, the
...
and real-world demos](/docs/reference/proof-matrix). For the serialized
