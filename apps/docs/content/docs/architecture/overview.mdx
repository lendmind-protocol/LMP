...
`lmp-core`. The daemon, CLI, and MCP server are delivery surfaces over that
same runtime; they are not independent policy engines.

Python and Node.js have deliberately narrower responsibilities. Python
orchestrates Docker execution, benchmark coordination, and analytics. Node.js
...
- `invalid_input` means the request or package could not be interpreted;
- `execution_error` means the runtime or an explicitly required execution
  boundary failed.
...
analytics](/docs/operations/benchmark-analytics), not in the architecture's
policy authority.
